'''
DQN的 max_q存在最大化偏差的问题，利用双重Q学习，消除最大化偏差。
基本思路：
      在DQN中使采用目标Q网络得到动作价值max_q来计算回报，U <- R + gamma * max(target_q(S', a, w_target))
      将最大值解耦，
            1. 利用评估Q网络获取最优动作max_a = argmax_a(q(S', a, w))，
            2. 然后将max_a代入到目标Q网络计算q值，再计算回报
            U <- R + gamma * target_q(S', argmax_a(q(S', a, w)), w_target)

1. 在200个episode内DDQN和DQN的差别不是很大， 影响更大的是对于reward的修改，更好的reward确实能够加速训练。
但是对于训练效果的进一步提升，目前来看，通过修改reward也难提升。
2. 利用双重q学习，解决最大值偏差也难以进一步提升训练效果，也即让reward突破200.
'''
    ...
    def train_Q_network(self):
        ...
        for x in minibatch:   # 对minibatch中每一条MDP数据循环
            if x[4]:          # 如果已经到达终止状态
                ...
                q_next = self.Q_network(next_state)  # 用当前q网络计算下一状态的动作值
                action_max = torch.argmax(q_next).item()  # 求最大动作值对应的动作， 此处获取action_max数值
                action_max_onehot = torch.eye(self.env.aspace_size)[action_max]  # action_max_onehot 相当于动作的序号，或者索引

                target_q_next = self.Q_network_t(next_state).squeeze(0)  # 用目标Q网络计算动作值
                target_q_next_max = torch.dot(target_q_next, action_max_onehot)  # 点积，得到一个数，在这里就利用近似的Q表，乘以动作的序号，得到动作对应的价值

                td_target = x[2]+self.env.gamma * target_q_next_max
