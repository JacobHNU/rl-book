# 这个代码写的看起来很迷糊，而且训练效果一直调不好，不知道怎么调
...
'''
class DQN2015():
    def __init__(self, env, epsilon=0.1, learning_rate=1e-2,
                 replay_size=2000, batch_szie=32, target_replace_iter=100):
        self.replay_buffer = deque()   # 初始化经验回放池
        self.env = env                 # 环境模型
        self.epsilon = epsilon         # epsilon-greedy策略的参数
        self.learning_rate = learning_rate  # 学习率
        self.repaly_size = replay_size      # 经验回放池最大容量
        self.batch_size = batch_szie        # 批量尺度
    ...
    def perceive(self, state, action, reward, next_state, done):
        ...
        # 如果经验回放池溢出，则删除最早经验数据
        if len(self.replay_buffer) > self.repaly_size:
    ...
    def train(self, num_episodes=400, num_steps=2000, render=False):
        ...
        for episode in range(num_episodes):
            ...
            # for step in range(num_steps):
                ...
                if (step + 1) % self.target_replace_iter == 0:  # 目标Q网络参数更新
